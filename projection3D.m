function [tdx,tdy,tdz] = projection3D(init_width,x,y,cluster_pix,x_dim,y_dim,FOV)
...
%FOV          is the field of view of the image
...
%%%%% Z axis
theta = asind(sqrt(cluster_pix/tot_pixels)*sind(FOV)); %calculate the angle field of view that the cluster takes up
tdz=init_width/tand(theta);  %calculate the z axis coordinate

%%%x and y axes

theta = asind(((x-(x_dim/2))/(x_dim/2))*sind(FOV)); %calculate the angle field of view that the cluster takes up
tdx=sind(theta)*tdz;  %calculate the z axis coordinate

theta = asind(((y-(y_dim/2))/(y_dim/2))*sind(FOV)); %calculate the angle field of view that the cluster takes up
tdy=sind(theta)*tdz;
